import { LineChart as ChartIcon, History, TrendingUp, TrendingDown, Minus } from "lucide-react";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

const PREDICTIONS_DATA = [
  { id: "PRD-0041", date: "2025-04-28", ampPerCycle: "1.2 A", hours: "8.5 hrs", result: "8.4 A", trend: "up" as const },
  { id: "PRD-0040", date: "2025-04-27", ampPerCycle: "0.9 A", hours: "6.0 hrs", result: "6.2 A", trend: "down" as const },
  { id: "PRD-0039", date: "2025-04-26", ampPerCycle: "1.1 A", hours: "7.0 hrs", result: "7.1 A", trend: "neutral" as const },
  { id: "PRD-0038", date: "2025-04-25", ampPerCycle: "1.3 A", hours: "9.0 hrs", result: "9.6 A", trend: "up" as const },
];
    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <Card key={stat.label} className="bg-[#111113] border-white/[0.06] p-4 rounded-xl">
            <div className="text-[10px] text-text-faint uppercase tracking-wider mb-2 font-medium">{stat.label}</div>
            <div className="flex items-end justify-between">
              <span className="text-xl font-display font-bold text-white">{stat.value}</span>
              <span className={`text-[10px] font-semibold font-mono ${stat.up ? "text-emerald-400" : "text-red-400"}`}>{stat.change}</span>
            </div>
          </Card>
        ))}
      </div>
        ...
        <Card className="lg:col-span-2 bg-[#111113] border-white/[0.06] p-4 rounded-xl flex flex-col">
          ...
          <div className="flex-1 min-h-[120px] relative">
            <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 60">
              ...
              <path d="M0,48 C6,45 10,38 16,35 C22,32 28,42 34,38 C40,34 46,22 52,25 C58,28 64,30 70,24 C76,18 82,20 88,14 C94,8 97,10 100,12 L100,60 L0,60 Z" fill="url(#previewChartFill)" />
              <path d="M0,48 C6,45 10,38 16,35 C22,32 28,42 34,38 C40,34 46,22 52,25 C58,28 64,30 70,24 C76,18 82,20 88,14 C94,8 97,10 100,12" fill="none" stroke="#C9A84C" strokeWidth="1.5" strokeLinecap="round" />
        ...
        <Card className="lg:col-span-3 bg-[#111113] border-white/[0.06] rounded-xl flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex flex-col">
            {PREDICTIONS_DATA.map((row, i) => (
              <div key={row.id} className={`grid grid-cols-6 gap-2 px-4 py-2.5 text-[11px] items-center ${i < PREDICTIONS_DATA.length - 1 ? "border-b border-white/[0.03]" : ""}`}>
                ...
                <span className="text-right">
                  <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[8px] px-1.5 py-0 rounded-full font-medium">{t("statusDone")}</Badge>
                </span>
              </div>
            ))}
          </div>
